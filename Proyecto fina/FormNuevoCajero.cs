...
{
    public partial class FormNuevoCajero : Form
    {
        int id_usuario_rol_pagina;
        public int guardado = 0;
        public FormNuevoCajero(int id_usuario)
        {
            id_usuario_rol_pagina = id_usuario_rol_pagina;
            InitializeComponent();
        }
        ...
        private void btn_registrar_Click(object sender, EventArgs e)
        {
            if (lbl_id_cajero.Text == "")
            {
                ...
                else
                {
                    try
                    {
                        ...
                        u.Id_usuario = uti.getUltimoIDUsuario("tb_usuario") + 1;
                        u.Id_tipo_rol = 2;
            ...
            else
            {
                ...
                else
                {
                    try
                    {
                        int id_usuario = Convert.ToInt32(lbl_id_cajero.Text);
                        u.Nuevo_nombre = txt_registro_nombre.Text.Trim().ToUpper();
                        u.Nuevo_apellido_paterno = txt_registro_apellido_paterno.Text.Trim().ToUpper();
                        u.Nuevo_apellido_materno = txt_registro_apellido_materno.Text.Trim().ToUpper();
                        u.Nuevo_sexo = cb_registro_sexo.Text.Trim().ToUpper();
                        u.Nuevo_fecha_nacimiento = dte_registro_fecha_nacimiento.Value.Year + "-" + dte_registro_fecha_ingreso.Value.Month + "-" + dte_registro_fecha_ingreso.Value.Day;
                        u.Nuevo_curp = txt_registro_curp.Text.Trim();
                        u.Nuevo_numero_nomina = txt_registro_numero_nominal.Text.Trim();
                        u.Nuevo_correo_electronico = txt_registro_email.Text.Trim();
                        u.Nuevo_contraseña = txt_registro_contraseña.Text.Trim();

                        if (UsuariosCAD.actualizarUsuario(u, id_usuario))
                        {
                            MessageBox.Show("Cajero actualizado exitosamente", "EXITO", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            this.Close();
                        }
                        else
                        {
                            MessageBox.Show("El cajero que intenta actualizar ya existe", "ADVERTENCIA", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        }
                    }
                    catch (Exception ex)
                    {
