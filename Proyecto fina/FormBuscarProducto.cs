...
namespace Proyecto_fina
{
    public partial class FormBuscarProducto : Form
    {
        ...
        private void btn_agregar_producto_carrito_Click(object sender, EventArgs e)
        {
            ...
            {
                if ((float)frm.getCantidadInventario(id_producto) >= (double)txt_cantidad_nombre_producto.Value)
                {
                    try
                    {
                        if (frm.existeProducto(id_producto))
                        {
                            if (frm.existeEnLista(id_producto))
                            {
                                for (int i = 0; i < frm.dg_carrito.RowCount; i++)
                                {
                                    ...
                                    {
                                        stock_temp -= (double)frm.dg_carrito.Rows[i].Cells[3].Value;
                                        ...
                                        {
                                            double cantidad = (double)frm.dg_carrito.Rows[i].Cells[3].Value + (double)txt_cantidad_nombre_producto.Value;
                            ...
                            else
                            {
                                ...
                                if (dt.Rows.Count > 0)
                                {
                                    ...
                                    cantidad = (double)txt_cantidad_nombre_producto.Value;
